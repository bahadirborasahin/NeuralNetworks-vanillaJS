...
MyMathcreateArray = function(row,col){
    return new Matrix(row,col);
}

MyMatharray = function(array){
    temp = new Matrix(array.length, array[0].length);
    temp.matrix = array;
    return temp;
}

MyMathdot = function (m1, m2){
    if(m1.col != m2.row){
        throw "Shape Error: ("+m1.row+","+m1.col+") dot ("+m2.row+","+m2.col+") is not possible";
    }
    else{
...
MyMathscale = function(m1,scalar){
    for(var i=0;i<m1.row;i++){
        for(var k=0;k<m1.col;k++){
            m1.matrix[i][k] *= scalar;
        }
